...
public final class MCCShotsListPageController: MCCViewController<MCCShotsListPageView, MCCEmptyViewModel> {
    ...
    public override func mcvc_bind() {
        ...
        contentView.mcvw_collectionView.dataSource = self
        contentView.mcvw_collectionView.delegate = self
        contentView.mcvw_collectionView.prefetchDataSource = self

        let header = MJRefreshNormalHeader { [weak self] in
            self?.mcvc_loadList(kind: .pullToRefresh)
        }
        header.lastUpdatedTimeLabel?.isHidden = true
        contentView.mcvw_collectionView.mj_header = header

        contentView.mcvw_collectionView.mj_footer = MJRefreshAutoNormalFooter { [weak self] in
